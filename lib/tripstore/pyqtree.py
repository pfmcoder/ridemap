...
## Patches by Patrick Maguire:

  intersect takes a groupby parameter that is a function that returns the 
      key to be used for the return value based on the item
  intersect takes a nodefilter parameter that is a function that allows for 
      applying a check to the item when determining whether to include it in the result set
"""
...
class _QuadTree:
  ...
  def _intersect(self, bbox, results=None, groupby=None, nodefilter=None):
    rect = bbox
    if results is None:
    ...
    if len(self.children) > 0 :
      if rect[0] <= self.center[0]:
        if rect[1] <= self.center[1]:
          self.children[0]._intersect(rect, results, groupby, nodefilter)
        if rect[3] >= self.center[1]:
          self.children[1]._intersect(rect, results, groupby, nodefilter)
      if rect[2] >= self.center[0]:
        if rect[1] <= self.center[1]:
          self.children[2]._intersect(rect, results, groupby, nodefilter)
        if rect[3] >= self.center[1]:
          self.children[3]._intersect(rect, results, groupby, nodefilter)
    # search node at this level
    for node in self.nodes:
      if (node.rect[2] >= rect[0] and node.rect[0] <= rect[2] and 
          node.rect[3] >= rect[1] and node.rect[1] <= rect[3]):
        if(nodefilter is None or nodefilter(node.item)):
          key = tuple(node.rect)
          if groupby is not None:
            key = groupby(node.item)
          results[key].append(node.item)
    return results
  ...
  def _countmembers(self, nodefilter=None):
    """
    Returns:
    ...
    for child in self.children:
      count += child._countmembers(nodefilter)
...
class Index(_QuadTree):
  ...
  def __init__(self, bbox, maxitems=10, maxdepth=20):
    ...
    - **maxitems** (optional): The maximum number of items allowed per quad before splitting
  ...
  def intersect(self, bbox, groupby=None, nodefilter=None):
    """
    ...
    - **groupby**: A one argument function that is passed the item and that provides the key for grouping 
    - **nodefilter**: A one argument function that is passed the item and returns True if it is to be in the result 
    Returns:

    - A dict of inserted items whose bounding boxes intersect with the input rectangle.
    """
    return self._intersect(bbox, groupby=groupby, nodefilter=nodefilter)
